...
try:
    ...
    def handle_button1(cycle):
        global is_display_off, is_sleeping, display, BRIGHTNESS
        ...
        if cycle > 3 and not is_display_off and not is_sleeping:
            ...
            display.message("BR" + str(BRIGHTNESS))
            time.sleep(1)
            display.set_brightness(BRIGHTNESS)
            save_settings([BRIGHTNESS, display.theme_id])
            render_time(now, True)
    ...
    def handle_button2(cycle):
        ...
        if cycle > 3 and not is_display_off and not is_sleeping:
            ...
            BRIGHTNESS += 1
            BRIGHTNESS = 10 if BRIGHTNESS > 10 else BRIGHTNESS
            display.message("BR" + str(BRIGHTNESS))
            time.sleep(1)
            display.set_brightness(BRIGHTNESS)
            save_settings([BRIGHTNESS, display.theme_id])
    ...
    display.set_brightness(settings[0])
    display.set_theme(settings[1])
